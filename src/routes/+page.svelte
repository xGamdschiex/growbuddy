...
<div class="px-4 pt-6 max-w-lg mx-auto space-y-6">
	...
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold">GrowBuddy</h1>
			<p class="text-gb-text-muted text-sm">Dein smarter Grow-Assistent</p>
		</div>
	...
	<div class="grid grid-cols-3 gap-3">
		<div class="bg-gb-surface rounded-xl p-3 text-center">
			...
			<p class="text-xs text-gb-text-muted">Aktive Grows</p>
		</div>
		<div class="bg-gb-surface rounded-xl p-3 text-center">
		...
		<div class="bg-gb-surface rounded-xl p-3 text-center">
			...
			<p class="text-xs text-gb-text-muted">Tage Streak</p>
		</div>
	</div>
	...
	{#if active.length > 0}
		<div class="space-y-2">
			<h2 class="text-sm font-semibold text-gb-text-muted uppercase tracking-wide">Aktive Grows</h2>
			{#each active as grow}
				...
				<a href="/grow/{grow.id}" class="block bg-gb-surface rounded-xl p-4 hover:bg-gb-surface-2 transition-colors">
					...
					{:else}
						<p class="text-xs text-gb-text-muted">Noch kein Check-in</p>
					{/if}
				</a>
			{/each}
		</div>
	...
	<!-- Quick Actions -->
	<div class="space-y-2">
		<h2 class="text-sm font-semibold text-gb-text-muted uppercase tracking-wide">Quick Actions</h2>
		<div class="grid grid-cols-2 gap-3">
			<a href="/grow/new" class="bg-gb-green/10 border border-gb-green/20 rounded-xl p-4 hover:bg-gb-green/20 transition-colors">
				...
				<p class="font-medium text-sm">Neuer Grow</p>
				<p class="text-xs text-gb-text-muted">Grow starten</p>
			</a>
			<a href="/calc" class="bg-gb-accent/10 border border-gb-accent/20 rounded-xl p-4 hover:bg-gb-accent/20 transition-colors">
				...
				<p class="font-medium text-sm">Düngerrechner</p>
				<p class="text-xs text-gb-text-muted">Nährstoffe berechnen</p>
			</a>
			<a href="/tools/vpd" class="bg-gb-info/10 border border-gb-info/20 rounded-xl p-4 hover:bg-gb-info/20 transition-colors">
				...
				<p class="font-medium text-sm">VPD Rechner</p>
				<p class="text-xs text-gb-text-muted">Klima optimieren</p>
			</a>
			<a href="/tools/dli" class="bg-gb-warning/10 border border-gb-warning/20 rounded-xl p-4 hover:bg-gb-warning/20 transition-colors">
				...
				<p class="font-medium text-sm">DLI Rechner</p>
				<p class="text-xs text-gb-text-muted">Licht optimieren</p>
			</a>
		</div>
	</div>
	...
	{#if harvested.length > 0}
		<div class="space-y-2">
			<h2 class="text-sm font-semibold text-gb-text-muted uppercase tracking-wide">Letzte Ernten</h2>
	...
	{#if active.length === 0 && harvested.length === 0}
		<div class="bg-gb-surface rounded-xl p-6 text-center">
			...
			<p class="text-gb-text-muted text-sm mb-3">Noch keine Grows — starte deinen ersten!</p>
