<script lang="ts">
	import { goto } from '$app/navigation';
	import { onboardingStore } from '$lib/stores/onboarding';
	import { proStore } from '$lib/stores/pro';

	let step = $state(0);
	let experience = $state<'beginner' | 'intermediate' | 'advanced' | null>(null);
	let goal = $state<'first_grow' | 'improve' | 'document' | 'commercial' | null>(null);

	const slides = [
		{
			icon: '🌱',
			title: 'Willkommen bei GrowBuddy',
			subtitle: 'Dein smarter Grow-Assistent — wissenschaftlich fundiert, offline-fähig, kostenlos.',
		},
		{
			icon: '📊',
			title: 'Tracke deine Grows',
			subtitle: 'Tägliche Check-ins mit Foto, VPD, EC/pH — alles an einem Ort. Behalte den Überblick über jede Pflanze.',
		},
		{
			icon: '🧪',
			title: 'Präzise Nährstoffberechnung',
			subtitle: '6 Düngerlinien, CalMag-Auto, Faktor-Smoothing — der genaueste Rechner auf dem Markt.',
		},
		{
			icon: '🏆',
			title: 'Werde besser mit jedem Grow',
			subtitle: 'XP, Level und Achievements basierend auf echten Aktionen. Grow Score zeigt dir, wo du stehst.',
		},
	];

	function next() {
		if (step < slides.length - 1) {
			step++;
		} else if (step === slides.length - 1) {
			step = slides.length; // Experience screen
		} else if (step === slides.length) {
			step = slides.length + 1; // Goal screen
		}
	}

	function finish() {
		onboardingStore.complete(experience, goal);
		goto('/');
	}

	function skip() {
		onboardingStore.complete(null, null);
		goto('/');
	}

	function startTrial() {
		proStore.startTrial();
		onboardingStore.complete(experience, goal);
		goto('/');
	}
</script>

<div class="min-h-screen flex flex-col items-center justify-center px-6 max-w-md mx-auto">

	{#if step < slides.length}
		<!-- Intro Slides -->
		<div class="text-center space-y-6 animate-[fadeIn_0.4s_ease-out]">
			<span class="text-6xl block">{slides[step].icon}</span>
			<h1 class="text-2xl font-bold leading-tight">{slides[step].title}</h1>
			<p class="text-gb-text-muted leading-relaxed">{slides[step].subtitle}</p>

			<!-- Dots -->
			<div class="flex justify-center gap-2 pt-4">
				{#each slides as _, i}
					<div class="w-2 h-2 rounded-full transition-all duration-300 {i === step ? 'bg-gb-green w-6' : 'bg-gb-border'}"></div>
				{/each}
			</div>

			<div class="pt-6 space-y-3">
				<button onclick={next}
					class="w-full bg-gb-green text-black font-semibold py-3.5 rounded-xl text-sm hover:bg-gb-green/80 transition-colors">
					{step === slides.length - 1 ? 'Los geht\'s' : 'Weiter'}
				</button>
				<button onclick={skip}
					class="w-full text-gb-text-muted text-sm py-2 hover:text-gb-text transition-colors">
					Überspringen
				</button>
			</div>
		</div>

	{:else if step === slides.length}
		<!-- Experience Level -->
		<div class="w-full space-y-6 animate-[fadeIn_0.4s_ease-out]">
			<div class="text-center">
				<h1 class="text-2xl font-bold">Wie erfahren bist du?</h1>
				<p class="text-gb-text-muted text-sm mt-2">Damit wir die App auf dich anpassen können</p>
			</div>

			<div class="space-y-3">
				{#each [
					{ val: 'beginner', icon: '🌱', title: 'Anfänger', desc: 'Erster Grow oder noch wenig Erfahrung' },
					{ val: 'intermediate', icon: '🌿', title: 'Fortgeschritten', desc: 'Einige Grows abgeschlossen, Basics sitzen' },
					{ val: 'advanced', icon: '🌳', title: 'Erfahren', desc: 'Viele Grows, optimiere Details' },
				] as opt}
					<button onclick={() => { experience = opt.val as typeof experience; next(); }}
						class="w-full bg-gb-surface rounded-xl p-4 text-left hover:bg-gb-surface-2 transition-colors border-2
							{experience === opt.val ? 'border-gb-green' : 'border-transparent'}">
						<div class="flex items-center gap-3">
							<span class="text-2xl">{opt.icon}</span>
							<div>
								<p class="font-semibold">{opt.title}</p>
								<p class="text-sm text-gb-text-muted">{opt.desc}</p>
							</div>
						</div>
					</button>
				{/each}
			</div>
		</div>

	{:else if step === slides.length + 1}
		<!-- Goal -->
		<div class="w-full space-y-6 animate-[fadeIn_0.4s_ease-out]">
			<div class="text-center">
				<h1 class="text-2xl font-bold">Was ist dein Ziel?</h1>
				<p class="text-gb-text-muted text-sm mt-2">Wir helfen dir, es zu erreichen</p>
			</div>

			<div class="space-y-3">
				{#each [
					{ val: 'first_grow', icon: '🌱', title: 'Ersten Grow starten', desc: 'Step-by-step zum ersten Ergebnis' },
					{ val: 'improve', icon: '📈', title: 'Ergebnisse verbessern', desc: 'Mehr Ertrag, bessere Qualität' },
					{ val: 'document', icon: '📋', title: 'Grows dokumentieren', desc: 'Alles an einem Ort festhalten' },
					{ val: 'commercial', icon: '🏢', title: 'Professioneller Anbau', desc: 'CSC oder kommerzieller Betrieb' },
				] as opt}
					<button onclick={() => { goal = opt.val as typeof goal; }}
						class="w-full bg-gb-surface rounded-xl p-4 text-left hover:bg-gb-surface-2 transition-colors border-2
							{goal === opt.val ? 'border-gb-green' : 'border-transparent'}">
						<div class="flex items-center gap-3">
							<span class="text-2xl">{opt.icon}</span>
							<div>
								<p class="font-semibold">{opt.title}</p>
								<p class="text-sm text-gb-text-muted">{opt.desc}</p>
							</div>
						</div>
					</button>
				{/each}
			</div>

			{#if goal}
				<div class="space-y-3 pt-2">
					<!-- Pro Trial CTA -->
					<div class="bg-gb-accent/10 border border-gb-accent/20 rounded-xl p-4 text-center">
						<p class="font-semibold text-sm">7 Tage Pro kostenlos testen</p>
						<p class="text-xs text-gb-text-muted mt-1">Unbegrenzte Grows, AI Diagnose, Cloud-Sync</p>
						<button onclick={startTrial}
							class="mt-3 bg-gb-accent text-white font-semibold text-sm px-6 py-2.5 rounded-lg hover:bg-gb-accent/80 transition-colors">
							Pro testen — kostenlos
						</button>
					</div>

					<button onclick={finish}
						class="w-full bg-gb-green text-black font-semibold py-3.5 rounded-xl text-sm hover:bg-gb-green/80 transition-colors">
						Mit Free starten
					</button>
				</div>
			{/if}
		</div>
	{/if}
</div>
