...
<div class="min-h-screen flex flex-col items-center justify-center px-6 max-w-md mx-auto">
	...
	{#if step < slides.length}
		...
		<div class="text-center space-y-6 animate-[fadeIn_0.4s_ease-out]">
			...
			<div class="pt-6 space-y-3">
				...
				<button onclick={skip}
					class="w-full text-gb-text-muted text-sm py-2.5 hover:text-gb-text transition-colors"
	...
	{:else if step === CLOUD_STEP}
		...
		<div class="w-full space-y-6 animate-[fadeIn_0.4s_ease-out]">
			...
			{#if oauthPending || finishing}
				<!-- Loading-State während OAuth läuft -->
				<div class="bg-gb-surface rounded-xl p-6 text-center space-y-3">
					<div class="w-10 h-10 border-3 border-gb-green border-t-transparent rounded-full animate-spin mx-auto"></div>
			...
			{:else}
				<div class="space-y-3">
					...
					<button onclick={cloudGoogle}
						class="w-full flex items-center justify-center gap-2 bg-white text-gray-700 font-semibold text-sm py-3.5 rounded-xl border border-gray-300 hover:bg-gray-50 transition-colors"
						style="min-height:48px">
					...
					<div class="space-y-2">
						<input type="email" bind:value={loginEmail} placeholder={tr('auth.email_placeholder')}
							...
							onkeydown={(e) => { if (e.key === 'Enter') cloudEmail(); }}
				...
				<button onclick={finish}
					class="w-full bg-gb-surface border border-gb-border text-gb-text-muted text-sm py-2.5 rounded-xl hover:text-gb-text transition-colors"
					style="min-height:44px">
